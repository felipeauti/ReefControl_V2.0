...
bool WebServerManager::begin(RelayController* relayController, ConfigManager* configManager, NtpClientManager* ntpClient, SensorManager* sensorManager) {
  ...
  _server.on("/api/sensors", [this]() {
    ...
    // Obtém a lista de sensores DS18B20
    String ds18b20Json = _sensorManager->getDS18B20SensorsJson();
    json += ds18b20Json;
  ...
  _server.on("/api/outputs", HTTP_GET, [this]() {
    ...
    // Itera sobre todas as saídas disponíveis
    for (int i = 0; i < _relayController->getNumOutputs(); i++) {
      ...
      if (_configManager) {
        ...
        json += "\"pin\":" + String(i + 4) + ","; // Pinos padrão começando do 4
        json += "\"name\":\"Saída " + String(i + 1) + "\",";
        json += "\"enabled\":true,";
        json += "\"autoMode\":false";
      }
      json += "}";
  ...
  _server.on("/api/outputs", HTTP_POST, [this]() {
    ...
    // Parse do JSON (formato: {"output":0,"state":true})
    ...
    if (_relayController->setOutput(outputIndex, state)) {
      String outputName = _configManager ? String(_configManager->relay.outputs[outputIndex].name) : "Saída " + String(outputIndex + 1);
      Serial.printf("✅ %s: %s\n", outputName.c_str(), state ? "LIGADA" : "DESLIGADA");
      _server.send(200, "application/json", "{\"success\":true}");
    } else {
  ...
  _server.on("/api/outputs/config", HTTP_POST, [this]() {
    ...
    if (_configManager->setOutputsFromJson(body)) {
      // Atualizar configuração do RelayController
      if (_relayController) {
        _relayController->updateConfig();
      }
